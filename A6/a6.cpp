...
int main()
{
	...
	do
	{
		...
		for(int m=0;m<k;m++) // cluster number
		{
			...
			cout<<"\nThe new centroids of cluster "<<m<<" are: ";
			cenx[m]=cenx[m]/xcnt;
			ceny[m]=ceny[m]/xcnt;
			cout<<cenx[m]<<","<<ceny[m]<<"\n";
		}
	...
	for(int m=0;m<k;m++) //for cluster number
	{
		cout<<"\nCluster "<<m+1<<" centre is : ("<<cenx[m]<<","<<ceny[m]<<")";
		cout<<"\nObjects are: ";
		for(int j=0;j<n;j++)
		{
			if(kc[j]==m) //if the cluster number is equal to i then print the elements i.e. present
			{
				cout<<"\n("<<x[j]<<","<<y[j]<<")";
			}
		}
